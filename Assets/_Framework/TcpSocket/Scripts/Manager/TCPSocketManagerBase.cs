...
public abstract class TCPSocketManagerBase<T> : MonoSingleton<T> where T : TCPSocketManagerBase<T>
{
            ...
            while (socket.Connected && isConnected)
            {
                try
                {
                    ...
                    while (processedLength < newBuffer.Length)
                    {
                        ...
                        // typeBytes 참고
                        // int16 2bytes
                        var roomStateByte = reader.ReadBytes(2); // Read 1 byte for RoomState
                        Array.Reverse(roomStateByte);
                        var roomState = (RoomState)BitConverter.ToInt16(roomStateByte);
                        Debug.Log($"RoomState:{roomState}");
                        var roomIdBytes = reader.ReadBytes(4);
                        Array.Reverse(roomIdBytes);
                        var roomId = BitConverter.ToInt32(roomIdBytes);
    ...
    public void Send(GamePacket gamePacket)
    {
        ...
        var packet = new Packet(SocketManager.instance.roomState, SocketManager.instance.roomId, gamePacket.PayloadCase, version, sequenceNumber++, byteArray);
